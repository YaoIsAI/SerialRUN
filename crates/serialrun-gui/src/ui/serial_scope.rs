        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.button(egui::RichText::new("?").size(14.0).strong())
                    .on_hover_text(T::scope_tip_header(lang));
            });
        });
        ui.add_space(4.0);
        ...
        while !stop.load(std::sync::atomic::Ordering::Relaxed) {
            match port.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    let interval = 0.1; // Distribute points across 0.1ms span
                    let points: Vec<ScopeDataPoint> = buf[..n].iter().enumerate().map(|(i, &byte)| {
                        ScopeDataPoint { time_ms: now + i as f64 * interval, value: byte as f64 }
                    }).collect();

...
pub fn render_flasher_panel(ui: &mut egui::Ui, state: &mut AppState) {
        ...
        if let Ok(result) = rx.try_recv() {
            ...
            match result {
                Ok(msg) => {
                    ...
                    if msg.contains("complete") || msg.contains("written") {
    ...
    ui.horizontal(|ui| {
        ui.heading(T::serial_flasher(lang));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.button(egui::RichText::new("?").size(14.0).strong())
                .on_hover_text(T::flasher_tip_header(lang));
        });
    });
    ...
    ui.horizontal(|ui| {
        ...
        if !state.flasher_file.is_empty() {
            let path = std::path::Path::new(&state.flasher_file);
            if path.exists() {
    ...
    // Progress — ASCII spinner animation
    if state.flasher_progress > 0.0 {
        let text = if state.flasher_progress >= 1.0 {
            T::flash_complete(lang).to_string()
        } else {
            format!("{}%", (state.flasher_progress * 100.0) as u32)
        };
        let color = if state.flasher_progress >= 1.0 { egui::Color32::from_rgb(34, 197, 94) } else { egui::Color32::from_rgb(59, 130, 246) };
        crate::ui::spinner::progress_with_spinner(ui, state.flasher_progress, &text, color);
    }
    ui.add_space(4.0);

    // Actions
    ui.horizontal(|ui| {
        let busy = state.flasher_async_receiver.is_some();
        if ui.add_enabled(!busy && state.is_connected, egui::Button::new(T::connect(lang))).clicked() {
            flasher_connect(state);
        }
        if ui.add_enabled(!busy && state.is_connected, egui::Button::new(T::erase(lang))).clicked() {
            flasher_erase(state);
        }
        if ui.add_enabled(!busy && state.is_connected && !state.flasher_file.is_empty(), egui::Button::new(T::flash(lang))).clicked() {
            flasher_flash(state);
        }
        if busy {
    ...
    if !state.flasher_log.is_empty() {
        ui.separator();
        let available = ui.available_height().max(80.0).min(300.0);
        egui::ScrollArea::vertical().max_height(available).stick_to_bottom(true).show(ui, |ui| {
            for line in &state.flasher_log {
                ...
                } else if line.contains("complete") || line.contains("OK") {
...
fn flasher_flash(state: &mut AppState) {
    ...
    std::thread::spawn(move || {
        ...
        let flasher = match mcu {
            crate::state::McuType::Stm32 => serialrun_core::protocol::flasher::Stm32Flasher::new(),
            crate::state::McuType::Esp32 => {
                let _ = tx.send(Err("ESP32 flashing not yet implemented. Use esptool.py for now.".into()));
                return;
            }
        };

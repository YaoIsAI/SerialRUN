...
pub struct SerialRunApp {
    ...
    hidden_viewports: std::collections::HashSet<egui::ViewportId>,
}
...
impl SerialRunApp {
    pub fn new(_cc: &eframe::CreationContext<'_>, mcp_handle: crate::mcp_server::McpHandle) -> Self {
        ...
        Self { state: state_clone, current_theme: Theme::Light, mcp_handle, last_prefs, hidden_viewports: std::collections::HashSet::new() }
    }
}
    ...
    while let Ok(req) = rx.recv() {
        match req {
            ...
            crate::mcp_server::McpSerialRequest::Read { timeout_ms, resume, resp } => {
                // Get rx_buffer Arc (brief lock), then poll directly (no state lock)
                let rx_buf = {
                    let state = lock_state(&app_state);
                    state.port_owner.as_ref().map(|po| po.rx_buffer_arc())
                };
                ...
                if !data.is_empty() {
                    ...
                    state.add_log_entry(crate::state::LogLevel::Info, &format!("MCP RX {} bytes", data.len()));
                }
            ...
            crate::mcp_server::McpSerialRequest::SendRead { data, timeout_ms, resp } => {
                let hex_preview = crate::ui::terminal::format_hex_bytes(&data);
                // Phase 1: Get rx_buffer Arc + Send write + display TX (lock held briefly)
                let rx_buf = {
                    let mut state = lock_state(&app_state);
                    if let Some(ref po) = state.port_owner {
                        po.send(crate::port_owner::PortCommand::Write(data.clone()));
                        Some(po.rx_buffer_arc())
                    } else {
                        None
                    }
                };
                ...
                // Phase 2: Poll rx_buffer directly (NO state lock needed — GUI stays responsive)
                let rx_buf = rx_buf.unwrap();
                let deadline = std::time::Instant::now() + std::time::Duration::from_millis(std::cmp::min(timeout_ms, 3000));
                let mut resp_data = Vec::new();
                while std::time::Instant::now() < deadline {
                    // Read directly from rx_buffer (has its own Mutex, no app_state lock)
                    if let Ok(mut buf) = rx_buf.lock() {
                        if !buf.is_empty() {
                            resp_data = buf.drain(..).collect();
                            break;
                        }
                    }
                    // Sleep WITHOUT holding any lock — GUI can process events freely
                    std::thread::sleep(std::time::Duration::from_millis(50));
                }
            ...
            crate::mcp_server::McpSerialRequest::SetConfig { key, value, resp } => {
                ...
                // Save prefs after config change
                if result.is_ok() {
                    crate::app::save_prefs(&crate::state::UserPrefs::from_state(&state));
                }
                let _ = resp.send(result);
            }
        }
...
impl eframe::App for SerialRunApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // --- Floating windows (multi-viewport) ---
        let use_viewports = !ctx.embed_viewports();
        macro_rules! viewport_window {
            ($show:expr, $id:expr, $title:expr, $render:expr, $w:expr, $h:expr) => {
                if $show {
                    if use_viewports {
                        let vid = egui::ViewportId(egui::Id::new($id));
                        let is_first_frame = !self.hidden_viewports.contains(&vid);
                        if is_first_frame {
                            self.hidden_viewports.insert(vid);
                        }
                        ctx.show_viewport_immediate(
                            ...
                            |ctx, _class| {
                                if ctx.input(|i| i.viewport().close_requested()) {
                                    $show = false;
                                    self.hidden_viewports.remove(&vid);
                                    return;
                                }
                                if is_first_frame {
                                    // First frame: render empty to avoid flash
                        ...
                        // Fallback for backends without multi-viewport support
                        egui::Window::new($title)
                            .default_width($w).default_height($h)
                            .show(ctx, |ui| { $render(ui, &mut state); });
                    }
                }
            };
        }

...
pub struct PortOwnerHandle {
    ...
    /// Dedicated buffer for MCP reads — avoids draining GUI's rx_buffer
    mcp_rx_buffer: Arc<Mutex<VecDeque<u8>>>,
}
...
impl PortOwnerHandle {
    pub fn start() -> Self {
        ...
        let mcp_rx_buffer: Arc<Mutex<VecDeque<u8>>> = Arc::new(Mutex::new(VecDeque::new()));
        let mcp_rbuf = mcp_rx_buffer.clone();
        let handle = thread::spawn(move || Self::run(cmd_rx, evt_tx, bcast, pname, rbuf, mcp_rbuf));
        Self { cmd_tx: Some(cmd_tx_inner), evt_rx, handle: Some(handle), broadcast_txs, port_name, rx_buffer, mcp_rx_buffer }
    }
    ...
    /// Get direct access to the MCP-specific rx_buffer Arc.
    /// MCP reads from this buffer instead of the main rx_buffer to avoid
    /// draining data that the GUI terminal also consumes.
    pub fn mcp_rx_buffer_arc(&self) -> Arc<Mutex<VecDeque<u8>>> {
        self.mcp_rx_buffer.clone()
    }
    ...
    fn run(cmd_rx: mpsc::Receiver<PortCommand>, evt_tx: mpsc::Sender<PortEvent>, broadcast_txs: Arc<Mutex<Vec<mpsc::Sender<PortEvent>>>>, port_name: Arc<Mutex<String>>, rx_buffer: Arc<Mutex<VecDeque<u8>>>, mcp_rx_buffer: Arc<Mutex<VecDeque<u8>>>) {
                    ...
                    Ok(cmd) => match cmd {
                        ...
                        PortCommand::ReadExclusive { data, timeout_ms, resp_tx } => {
                            if let Some(ref mut p) = port {
                                ...
                                read_paused = true;
                                // Clear MCP-specific buffer to prevent stale data
                        ...
                        PortCommand::WriteAndPause(data) => {
                            if let Some(ref mut p) = port {
                                ...
                                // Also clear MCP-specific buffer to prevent stale data
                                if let Ok(mut mcp_rbuf) = mcp_rx_buffer.lock() {
                                    mcp_rbuf.clear();
                                }
                                match p.write(&data) {
